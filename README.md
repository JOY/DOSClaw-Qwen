...
```
Web chat UI (tenant/customer selector + analytics, consent, handoff, memory, and knowledge controls)
        |  HTTP / SSE
FastAPI  --  AgentScope Agent + Mem0Middleware (Qwen qwen3.6-plus via DashScope)
        ...
        |        |
   Postgres + pgvector  (tenants, customers, profile, knowledge, handoffs) [on Alibaba Cloud]
        |
...
knowledge search, consent-aware profile recording, a staff handoff dashboard, support analytics,
FastAPI endpoints, and the web demo UI are in place. The public
