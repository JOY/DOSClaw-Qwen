...
- Mem0 runs in `both` mode: automatic recall/write-back plus agent-controlled `search_memory` and `add_memory` tools.
...
- The UI includes Memory controls and a Knowledge base panel so judges can inspect the underlying memory and tenant FAQ surfaces directly.
